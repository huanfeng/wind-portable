...
- [ ] 托盘右键「退出」在服务运行/已注册时弹三态确认（是=停服卸载 / 否=仅关启动器 / 取消=不退出）。
- [ ] 确认框选「否」→ 仅关闭启动器，IME 服务继续运行（符合输入法直觉）。
      **重点**：此时服务仍在跑，必须真的退出，不能被"收进托盘"的逻辑截走。
- [ ] 确认框选「是」→ 停服 + 移除自启 + 注销 TSF（可能弹 UAC）后退出；再开启动器状态为「已停止」。
- [ ] 确认框选「取消」→ 启动器不退出；**随后再点标题栏 X 应恢复"收进托盘"行为**（退出意图已作废）。
